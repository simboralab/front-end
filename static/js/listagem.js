...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // dados de exemplo - substitua por backend quando disponível
  ...
  function renderFilters() {
    ...
    // usar lista fixa de categorias conforme solicitado
    const categories = ['Esporte','Lazer','Cultura','Tecnologia','Educação'];
    const cities = uniqueValues(sampleData, 'city');
    ...
    function makePill(key, label, options, selectedValue) {
      ...
      // se opções vierem com contagem (ex.: categorias), calcule as contagens
        ...
        if (key === 'category' || key === 'city') {
          options.forEach(opt => {
            ...
            counts[v] = sampleData.filter(i => (key === 'category' ? i.category : i.city) === v).length;
          });
        } else if (key === 'status') {
          options.forEach(opt => {
            const v = opt.value ?? opt;
            if (!v) {
              counts[v] = sampleData.length;
            } else if (v === 'novo') {
              counts[v] = sampleData.filter(i => i.badges && i.badges.includes('Novo')).length;
            } else if (v === 'esgotado') {
              counts[v] = sampleData.filter(i => i.badges && i.badges.includes('Esgotado')).length;
            } else if (v === 'disponivel') {
              counts[v] = sampleData.filter(i => i.badges && i.badges.includes('Disponível')).length;
            } else {
              counts[v] = 0;
            }
          });
      ...
      if (key === 'category'){
        ...
        footer.querySelector('.btn-visualizar').addEventListener('click',(e)=>{ e.stopPropagation(); closeAllDropdowns(); resetAndRender(); });
        footer.querySelector('.btn-limpar').addEventListener('click',(e)=>{ e.stopPropagation(); filters.category = ''; updatePillValue(pill,'',''); ul.querySelectorAll('.option-checkbox').forEach(cb=> cb.checked = false); updateURL({category:''}); resetAndRender(); closeAllDropdowns(); });
  ...
  function applyAndRender() {
    filtered = applyFilters(sampleData, filters, mode);
    currentPage = 1;
    renderCardsPage();
  }

  function resetAndRender() { currentPage = 1; applyAndRender(); window.scrollTo({top:0,behavior:'smooth'}); }
  ...
  function createCard(item) {
    ...
    share.addEventListener('click', (e)=>{
      e.stopPropagation();
      const shareUrl = `${window.location.origin}${window.location.pathname.replace(/[^/]*$/,'') }event-details.html?id=${item.id}`;
      if (navigator.share) {
    ...
    card.addEventListener('click', ()=> { window.location.href = `event-details.html?id=${item.id}`; });
    card.addEventListener('keydown', (e)=> { if (e.key === 'Enter') window.location.href = `event-details.html?id=${item.id}`; });
  ...
  function applyFilters(data, f, mode) {
    return data.filter(item => {
      if (f.category && item.category !== f.category) return false;
      if (f.status) {
        if (f.status === 'novo' && !(item.badges && item.badges.includes('Novo'))) return false;
        if (f.status === 'esgotado' && !(item.badges && item.badges.includes('Esgotado'))) return false;
        if (f.status === 'disponivel' && !(item.badges && item.badges.includes('Disponível'))) return false;
      }
      if (f.q) {
        const qlow = f.q.toLowerCase();
        if (!item.title.toLowerCase().includes(qlow) && !item.description.toLowerCase().includes(qlow)) return false;
      }
      if (f.city && item.city !== f.city) return false;
      if (f.price === 'free' && item.price !== 0) return false;
      if (f.price === 'paid' && item.price === 0) return false;
      return true;
    });
  }
  ...
  // cria dados de exemplo simples
  function generateSampleData(n) {
    const cats = ['Esporte','Lazer','Cultura','Tecnologia','Educação'];
    ...
    for (let i=1;i<=n;i++) {
      ...
      const dayEnd = dayStart + 1;
        ...
        id: i,
        title: `${c} Evento ${i}`,
        description: `Descrição resumida do evento ${i}`,
        date: `${dayStart} de Dez a ${dayEnd} de Dez`,
        city: city,
        venue: `Arena ${(i%10)+1}`,
        category: c,
        provider: `Organizador ${((i%6)+1)}`,
        price: (i%4===0)?0: (20 + (i*3)),
        image: `https://picsum.photos/seed/event${i}/800/450`,
        badges: (i%6===0)?['Esgotado']: (i%5===0)?['Novo']: ['Disponível']
      });
    }
    return out;
  }
